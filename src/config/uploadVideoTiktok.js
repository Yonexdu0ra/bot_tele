export default (path_video, content) => ({
    creator: 'creator#',
    input: 'input[type=file]',
    path_video,
    preview: ".jsx-308560071.video-player-control",
    form: `div[data-offset-key].public-DraftStyleDefault-block.public-DraftStyleDefault-ltr`,
    button_upload: "button.css-y1m958",
    button_editVideo: "button.css-ug6w5y",
    span: `span[data-text]`,
    button_close_modal: ".jsx-591861640.modalCloseButton",
    button_close_split_video: "div.jsx-366267946.modal-header-btn-group > button.css-170cvvi",
    modal: 'div.tiktok-modal__modal-title',
    content
})